...
class HomeScreen(Frame):
    def __init__(self, master):
        ...
        serviceFile_Options= ["Zoom", "Teams"]
        ...
        canvas.create_text(
            ...
            210.0,
            anchor="nw",
            text="An attendance system for online meeting platforms (Zoom & Teams)\n"
                 "which uses meeting reports generated by these services\n"
                 "to automatically create and maintain records for the user.",
            fill="#000000",
        ...
        def call_data_processor(class_name: str = None, meeting_file: str = None, duration = None, cut_off= None, service_type: str = None):
            if class_options == [None]:
                ...
                if None in [class_name, meeting_file, cut_off, service_type]:
                    ...
                    global dp
                    dp = DataProcessor.DataProcessor(class_name, path + "/backend/Excel files/"+ class_name +".xlsx", meeting_file, service_type, duration, cut_off)
                    dp.output_to_text_file()
                    dp.output_to_console()
                    settings = open(path + "/backend/settings.txt", "w")
                    settings.write(class_name)
                    settings.close()
